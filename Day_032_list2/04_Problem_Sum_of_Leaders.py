'''
04_Problem: Sum of Leaders in an Array After Filtering Invalid Data (Python)
...
for i in arr:
   if i > 0:
      f_arr.append(i)
print("Filtered Array:",f_arr)

leadres = []
sum = 0                 
for i in range(len(f_arr)):
    isleader = True
    for j in range(i+1,len(f_arr)):
        if f_arr[i] <= f_arr[j]:
            isleader = False
            break
    if isleader:
        leadres.append(f_arr[i])
        sum = sum + f_arr[i]   

print("Leaders: ",leadres)
print("Sum: ",sum)   
